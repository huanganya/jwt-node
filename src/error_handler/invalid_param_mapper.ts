import { InvalidParam } from "./types";

const fromValidateJSError = (validateJSErrors: ValidateJsErrorFormat): InvalidParam[] => {
...
const fromExpressValidatorFormat = (errors: ExpressValidatorErrors): InvalidParam[] => {
    return errors.map((error) => {
        return { name: error.param, reason: error.msg };
    });
};
const toExpressValidatorFormat = (invalidParms: InvalidParam[]): ExpressValidatorErrors => {
