...
exports.audioAes256CbcDecryption = async function(
    ...
    const decipher = Crypto.createDecipheriv("aes-256-cbc", buffKeyValue, buffIVValue);
...
exports.audioPassword = async function(reqPassword){
    ...
    const EncryptedInputCheckSum = hexEncryptedInput.substr(hexEncryptedInput.length - 64, 64);
    // Extract Payload
    const hexEncryptedPayload = hexEncryptedInput.substr(0, hexEncryptedInput.length - EncryptedInputCheckSum.length);
    ...
    const PayloadSha = Crypto.createHash("Sha256").update(HexPayloadWithSalt).digest("hex");
    ...
    if (PayloadSha === EncryptedInputCheckSum) {
        ...
        const PayLoadValue = hexEncryptedPayload.substr(32, hexEncryptedPayload.length - 32 - 64);
        ...
        const PayloadKey = hexEncryptedPayload.substr(PayloadIV.length + PayLoadValue.length, 64);
